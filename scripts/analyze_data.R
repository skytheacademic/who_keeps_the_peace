...
# By: Sky Kunkel
#
# This script is organized to follow the order of the paper (main.tex). Each figure/table is
...
# PRIO grid cell. Moved here from plot_data.R so the paper's figures are reproduced by the
# analysis pipeline. Uses a fresh read so the analysis data frames `a`/`c` are left untouched.
...
# drop map-only objects (keep analysis data frames a, c)
...
#### Appendix ####
# NOTE: no rm() here -- se_reg9..se_reg12 are still needed for Table [tab:hyp_3].
