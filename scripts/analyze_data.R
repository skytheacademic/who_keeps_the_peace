...
reg9 = glm(ucdp_reb_vac_5 ~ t_bal + t_unbal + t_nobal + prio_mountains_mean + prio_ttime_mean + prio_urban_gc +
             radpko_pko_lag + viol_6,
           data = a, family = negative.binomial(theta = 1))
names(reg9$coefficients) = c("(Intercept)", "Gender-mixed PK Unit", "Unbalanced PK Unit", "All-male PK Unit",
                             "Avg. Mountain", "Travel Time Nearest City", "Perc. Urban",
                             "PK Lag", "Violence 6 Months Before")
se_reg9 <- round(coeftest(reg9, vcov = vcovPL(reg9, cluster = a$prio.grid)),4)
se_reg9

reg10 = glm(ucdp_reb_vac_all ~ t_bal + t_unbal + t_nobal + prio_mountains_mean + prio_ttime_mean + prio_urban_gc +
              radpko_pko_lag + viol_6,
            data = a, family = negative.binomial(theta = 1))
names(reg10$coefficients) = c("(Intercept)", "Gender-mixed PK Unit", "Unbalanced PK Unit", "All-male PK Unit",
                              "Avg. Mountain", "Travel Time Nearest City", "Perc. Urban",
                              "PK Lag", "Violence 6 Months Before")
...
stargazer(se_reg9, se_reg10, se_reg11, se_reg12, style = "APSR", title = "Models Testing Hypothesis 3",
          ...
          notes = c("The unmatched models distinguish all-male units from grid-months with no deployment;",
                    "the matched models drop all-male units entirely, since matching is defined only over deployments where women were present.")) # Logit analyses
